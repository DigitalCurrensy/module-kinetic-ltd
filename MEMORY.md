...
- DSN drain CLOSED: FakeConn 7 then 0; Timescale miss rolls back; unset URL writes 0.
- Desk CSMS: NotifyEvent 2.1 Alerting → lockout → work order → seven rows. 1.6 and NotifyPeriodicEventStream refuse with 0 writes. receipt.py 49eea447 frozen. http.py 1085a760 already on child.
- W0–W6 CLOSED on the desk. Site-scale happy path still clears; test_site_scale refuses r>1 MW.
- Drain ALLOWED_KINDS includes time_pin + commitment_run. No DROP/TRUNCATE.
